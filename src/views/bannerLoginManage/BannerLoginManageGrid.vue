<template>
  ...
  <BannerLoginImageSavePopup
    ref="bannerLoginImageGridRef"
    @onPopupSaveComplete="onSizeChangePagination"
  />
</template>
...
import BannerLoginImageSavePopup from './popup/BannerLoginImageSavePopup.vue'
...
const onGridCellClick = (event) => {
  if (event.colDef.field === 'name') {
    bannerLoginImageGridRef.value.open(event.data.id)
  }
}
/** Paging 관련 Event **/
...
const onPopupAddBtnClick = () => {
  bannerLoginImageGridRef.value.open(null)
}
...
const setPopupInfo = () => {
  ...
  gridItems.value = props.data.bannerLoginImageList // 조회해온 목록을 넣는다.
}
